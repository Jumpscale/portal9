...
class system_alerts(j.tools.code.classGetBase()):
    ...
    def __init__(self):
        ...
        self.appname = "system"
        self.alertmodel = j.data.models_system.Alert
    ...
    def _update(self, state, alert, comment=None, username=None, **kwargs):
        ...
        the store in db
        """
        if not j.data.models_system.Alert.exists(alert):
            raise RuntimeError('Invalid Alert')

        alert_obj = j.data.models_system.Alert.get(alert)

        if username and not j.data.models_system.Alert.find({'username': username})[0]:
