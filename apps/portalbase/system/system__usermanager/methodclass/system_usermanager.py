...
class system_usermanager(j.tools.code.classGetBase()):
    ...
    def userget(self, name, **kwargs):
        ...
        """
        user = j.data.models_system.User.find({"name": name, "gid": j.application.whoAmI.gid})
    ...
    def getuserwithid(self, id, **kwargs):
        ...
        param:id id of user
        """
        return j.data.models_system.User.get(id).to_dict()
    ...
    def getgroup(self, id, **kwargs):
        ...
        param:id id of user
        """
        return j.data.models_system.Group.get(id).to_dict()

    def listusers(self, **kwargs):
        dict_users = list()
        users = j.data.models_system.User.find({})
        for user in users:
    ...
    def _getUser(self, user):
        users = j.data.models_system.User.find({"name": user, "gid": j.application.whoAmI.gid})
        if not users:
            return None
        return users[0]
    ...
    def delete(self, username, **kwargs):
        ...
        user = j.data.models_system.User.find({"name": username})[0]
        user.delete()
        return True
    ...
    def deleteGroup(self, id, **kwargs):
        ...
        for user in j.data.models_system.User.find({"groups": group['name']}):
    ...
    def createGroup(self, name, domain, description, **args):
        ...
        """
        if j.data.models_system.Group.find({"name": name}):
            raise exceptions.Conflict("Group with name %s already exists" % name)
        group = j.data.models_system.Group()
    ...
    def editGroup(self, name, domain, description, users, **args):
        ...
        """
        groups = j.data.models_system.Group.find({"name": name})
        ...
        if users and isinstance(users, str):
            users = users.split(',')
        users_old = [u['name'] for u in j.data.models_system.User.find({'groups': name})]
    ...
    def _checkUser(self, username):
        ...
        users = j.data.models_system.User.find({"name": username})
        if not users:
    ...
    def userexists(self, name, **args):
        ...
        """
        user = j.data.models_system.User.find({"name": name, "gid": j.application.whoAmI.gid})[0]
        if user:
            return True
