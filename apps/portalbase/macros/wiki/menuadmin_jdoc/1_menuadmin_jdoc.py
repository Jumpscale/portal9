...
# ReloadAll:javascript:(function loadAll() {$.ajax({'url':
# '/system/ReloadApplication'});(function checkSpaceIsUp(trials) {if
# (trials <= 0) return;setTimeout(function() {$.ajax({'url':
# '/system/'}).done(function(){location.reload();console.log('Reloaded');}).error(function(){checkSpaceIsUp(trials
# - 1)});}, 1000);})(10);})();void(0);
